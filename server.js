      ...
      await new Promise((resolve, reject) => {
        connection.query(
          ...
          (error, results) => {
            if (error) {
              console.log(
                "Error while querying the database for agreementPrice in buySelectedAgreements: ",
                error
              );
              reject(error);
            } else {
          ...
          connection.query(
            ...
            (error, results) => {
              if (error) {
                ...
                console.log(
                  "No result found for agreement name: ",
                  agreementName
                );
                reject(new Error("No matching agreement found"));
              } else {
                console.log(
                  "Agreement price: ",
                ...
                if (
                  ...
                  console.log(
                    ...
                    selectedAgreementsPrices[index]
                  );
                  res.status(400).json({
                    status: "error",
                    message:
      ...
      await new Promise((resolve, reject) => {
        connection.query(
          ...
          (error, results) => {
            if (error) {
              console.log(
                "Error while querying the database (updating order status in OrderedProducts): ",
                error
              );
              reject(error);
            } else {
          ...
          const wasSent = await new Promise((resolve, reject) => {
            connection.query(
              ...
              (error, results) => {
                if (error) {
                  console.log(
                    "Error while querying the database (updating email sent status): ",
                    error
                  );
                  reject(error);
                } else {
...
app.post("/makePaymentForSubscription", async (req, res) => {
  try {
    ...
    //Retrieve the selected agreements' names and prices from the request body
    let selectedAgreementsIds = req.body;
    ...
    //Now verify if the subscription is the best one
    if (isBestSubscription === "Platinum") {
      //If it is, then the number of selected agreements' Ids does not matter
      console.log("Subscription is the best one");

      //We need to fill the seledctedAgreementsIds array with all the available agreements' Ids
      //Query the database to retrieve all the available agreements' Ids
      selectedAgreementsIds = await new Promise((resolve, reject) => {
        connection.query(
          "SELECT agreementId FROM Agreements",
          (error, results) => {
            if (error) {
              console.log("Error while querying the database", error);
              reject(error);
              ...
              console.log(
                "All the available agreements' Ids have been retrieved from the database"
              );
              resolve(results);
            }
          }
      ...
      if (
        ...
        numberOfAgreementsInSubscriptionFromDatabase
      ) {
        console.log(
    ...
    const subscriptionPrice = await new Promise((resolve, reject) => {
      connection.query(
        ...
        (error, results) => {
          if (error) {
            console.log(
              "Error while querying the database for subscription price",
              error
            );
            reject(error);
          } else {
    ...
    const subscriptionName = await new Promise((resolve, reject) => {
      connection.query(
        ...
        (error, results) => {
          if (error) {
            console.log(
              "Error while querying the database for subscription name",
              error
            );
            reject(error);
          } else {
    ...
    const selectedAgreementsNames = await new Promise((resolve, reject) => {
      connection.query(
        ...
        (error, results) => {
          if (error) {
            console.log(
              "Error while querying the database for selected agreements' names",
              error
            );
            reject(error);
          } else {
    ...
    const emailHash = crypto
      .createHash("sha256")
      .update(req.session.passport.user.email)
      .digest("hex");
    const emailPart = emailHash.substring(0, 8);
    ...
    const subscriptionPaymentNotifyUrl =
      process.env.SUBSCRIPTION_PAYEMENT_NOTIFY_URL;
    //Construct the notifyUrl
    ...
    try {
      ...
      let emailOptions = {
        ...
        text:
          "Dziękujemy za złożenie zamówienia w naszym serwisie internetowym. Numer zamówienia: " +
          extOrderId +
          "W przypadku pytań prosimy o kontakt na adres:" +
          "pomoc@prawokosmetyczne.pl",
      };

      transporter.sendMail(emailOptions, (error, info) => {
        if (error) {
          console.log(
            "Error during email sending regarding subscription order: ",
            error
          );
        } else {
          console.log(
            "Email regarding subscription order sent: ",
            info.response
          );
        }
      });
